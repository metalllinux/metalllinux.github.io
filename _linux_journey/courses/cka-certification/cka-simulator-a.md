---
title: "Cka Simulator A"
category: "cka-certification"
tags: ["cka-certification", "cka", "simulator"]
---

CKA Simulator A

* Runs on Kubernetes 1.33
...
SSH instances
Each question needs to be solved on a specific instance other than your main candidate@terminal. You'll need to connect to the correct instance via ssh, the command is provided before each question.

To connect to a different instance you always need to return first to your main terminal by running the exit command, from there you can connect to a different one.
...
Difficulty
The difficulty is comparable to the real exam, but it can depend on the questions and topics you get.
...
Content
All simulator sessions of the same type (CKA-A|CKA-B|CKS|CKAD|LFCS|LFCT) will have identical content/scenarios.
...
kubectl kustomise /opt/course/5/api-gateway/staging | kubectl apply -f -
kubectl kustomise /opt/course/5/api-gateway/prod | kubectl apply -f -
Using the Kustomise config at /opt/course/5/api-gateway do the following:
...
- name: account-0027@internal
  user:
    client-certificate-data: <base64-encoded-client-cert>    client-key-data: <base64-encoded-client-key>...
...
...
deploy: Deployments (manages stateless application pods)
ds: DaemonSets (ensures pods run on all or specific nodes)
sts: StatefulSets (manages stateful application pods with persistent storage)
...
Question 5 | Kustomise configure HPA Autoscaler
...
Answer
Kustomise is a standalone tool to manage K8s Yaml files, but it also comes included with kubectl. The common idea is to have a base set of K8s Yaml and then override or extend it for different overlays, like here done for staging and prod:
...
➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise base
apiVersion: v1
kind: ServiceAccount
metadata:
  name: api-gateway
...
Running kubectl kustomise DIR will build the whole Yaml based on whatever is defined in the kustomisation.yaml.
...
➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise staging
apiVersion: v1
kind: ServiceAccount
metadata:
  name: api-gateway
...
This all happens because of the kustomisation.yaml:

# cka5774:/opt/course/5/api-gateway/staging/kustomisation.yaml
apiVersion: kustomise.config.k8s.io/v1beta1
kind: Kustomisation
...
➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise staging | kubectl diff -f -

➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise staging | kubectl apply -f -
...
➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise prod
apiVersion: v1
kind: ServiceAccount
metadata:
  name: api-gateway
...
➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise prod | kubectl diff -f -

➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise prod | kubectl apply -f -
serviceaccount/api-gateway unchanged
configmap/horizontal-scaling-config unchanged
deployment.apps/api-gateway unchanged
...
➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise staging | grep maxReplicas -B5
kind: HorizontalPodAutoscaler
metadata:
  name: api-gateway
  namespace: api-gateway-staging
...
➜ candidate@cka5774:/opt/course/5/api-gateway$ k kustomise prod | grep maxReplicas -B5
kind: HorizontalPodAutoscaler
metadata:
  name: api-gateway
  namespace: api-gateway-prod
...
This means each time we deploy our Kustomise built Yaml, the replicas that the HPA applied would be overwritten, which is not cool. It does not matter for the scoring of this question but to prevent this we could simply remove the replicas: setting from the Deployment in base, staging and prod.

 

Kustomise / Helm and State
We had to delete the remote ConfigMaps manually. Kustomise won't delete remote resources if they only exist remote. This is because it does not keep any state and hence doesn't know which remote resources were created by Kustomise or by anything else.
...
Kustomise is less complex by not having to manage state, but might need more manual work cleaning up
...
● kubelet.service - kubelet: The Kubernetes Node Agent
    ...
    Process: 13058 ExecStart=/usr/bin/kubelet $KUBELET_KUBECONFIG_ARGS $KU>
...
➜ / # curl -k https://kubernetes.default/api/v1/secrets -H "Authorisation: Bearer ${TOKEN}"
{
  "kind": "SecretList",
  "apiVersion": "v1",
  "metadata": {
...
➜ / # curl -k https://kubernetes.default/api/v1/secrets -H "Authorisation: Bearer ${TOKEN}" > result.json
...
➜ / # curl --cacert ${CACERT} https://kubernetes.default/api/v1/secrets -H "Authorisation: Bearer ${TOKEN}"
...
role.rbac.authorisation.k8s.io/processor created
...
rolebinding.rbac.authorisation.k8s.io/processor created
...
<body>
<centre><h1>404 Not Found</h1></centre>
<hr><centre>nginx</centre>
</body>
</html>
...
spec:
  ...
  - command:
    ...
    - --authorisation-kubeconfig=/etc/kubernetes/controller-manager.conf
