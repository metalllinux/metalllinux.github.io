---
title: "How To Setup A Freeipa Server On Rocky Linux 9 (Do"
...
* Requires both an A record pointing to the IP address of the node and a CNAME record with `Host` set as `www` and a `Value` of `NAME.DOMAIN.SITE`
...
45.76.194.188    ipa.metalinux.site  ipa
EOF
```
...
```
* Installed the `FreeIPA` packages:
```
